class RedBlackTree<T : Comparable<T>> {
    ...
    private fun isBlack(node: RedBlackNode<T>?): Boolean {
    ...
    private fun insertFixup(nodeToFix: RedBlackNode<T>?) {
        ...
        while (node?.parent != null && isRed(parent)) {
            ...
            if (parent == gparent?.left) {
                val uncle: RedBlackNode<T>? = gparent?.right // Получить узел дяди
                // case1: узел дяди тоже красный
                if (uncle != null && isRed(uncle)) {
                    setBlack(parent) // Очернить родительский узел и узел дяди
                    setBlack(uncle)
                ...
                val uncle: RedBlackNode<T>? = gparent?.left // Получить узел дяди
                // case1: узел дяди тоже красный
                if (uncle != null && isRed(uncle)) {
                    setBlack(parent)
                    setBlack(uncle)
    ...
    **/
    private fun leftRotate(x: RedBlackNode<T>?) {
        ...
        if (x.parent == null) {
            ...
            else x.parent?.right = y // в противном случае установите y как правый дочерний узел
        }
    ...
    **/
    private fun rightRotate(y: RedBlackNode<T>?) {
        ...
        if (y.parent == null) {
            ...
            else y.parent?.left = x // в противном случае установите y как правый дочерний узел
        }
